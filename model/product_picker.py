...
class ProductPicker:
    ...
    def pick(
        ...
        if len(df) == 0:
            self.logger.warning(
                f"No products found in category {categories}, {price_range=}"
            )
            return []
        ...
        for product_id in products_response:
            ...
            if df_row.empty:
                self.logger.error(
                    f"Product ID {product_id} not found in any category of {categories}"
                )
                continue
    ...
    def _find_products(
        self, categories: list[str], price_range: tuple[float, float] = None
    ) -> tuple[pd.DataFrame, str]:
        df = self.df[self.df["category"].isin(categories)]
        if price_range is not None:
            df = df[
                (df["price_lower"] >= price_range[0])
                & (df["price_upper"] <= price_range[1])
            ]
        if len(df) > 50:
            df = df.sample(50)
...
if __name__ == "__main__":
    ...
    picker.pick(
        ["Toys & Games", "Stationery", "Fashion"],
        "A gift for a friend's birthday, who loves to read Harry Potter.",
    )
